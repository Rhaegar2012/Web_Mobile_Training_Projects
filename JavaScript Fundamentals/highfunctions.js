//High level fuynction to repeatr a number of times , we pass a function as a parameter for the repeat function
let labels=[]
...
//High order function to create functions , the return value is an arrue function 
...
if (typeof global != "undefined" && !global.SCRIPTS)
  ...
  //Filter the scripts array for only those for whose direction is rtl
  let rtlScripts = SCRIPTS.filter(s=>s.direction=="rtl");
  //Map creates a new array from the filtered scripts , aggregated only the name 
  console.log(map(rtlScripts,s=>s.name));

  //Summarizing with reduce
  //Reduce computes a single value from the values of an array , summing a collection of numbers is an example 
  function reduce(array, combine,start){
    let current = start;
    for(let element of array){
      combine(current,element);
    }
    return current;
  }
  console.log(reduce([1,2,3,4],(a,b)=>a+b,0));
  //using reduce twice wto find the script with the most characters 
  function characterCount(script){
    //In this case reduce is being called on the ranges parameter of a script directly (first argument) the second argument is a function that takes the count and the delta of to and from
    return script.ranges.reduce((count,[from,to])=>
    {
      return count+(to-from);},0);
  }
...
//The line below performs the following 
//Passes SCRIPTS to find the average based on the year of origin
//Filters scripts by whether the script is living or not 
//Creates a new array of years of origin for the lviing scripts 
//The array passed to average is an array of years only 
console.log(Math.round(average(SCRIPTS.filter(s=>s.living).map(s=>s.year))));
//Modify so passes dead scripts 
console.log(Math.round(average(SCRIPTS.filter(s=>!s.living).map(s=>s.year))));
